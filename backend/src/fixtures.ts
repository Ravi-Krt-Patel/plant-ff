...
export const seedProducts: Product[] = seeds.map(
  ([name, category, price, image, tag], i) => ({
    ...
    size: i % 2 ? "small" : "medium",
    stock: i === 13 ? 0 : i === 5 ? 3 : 10,
    version: 1,
    demo: true,
    description: `Sample ${name}. Photography and care data are illustrative.`,
    variants: [
      {
        id: "nursery",
        sku: `p${i + 1}-nursery`,
      ...
      },
      {
        id: "ceramic",
        sku: `p${i + 1}-ceramic`,
        pricePaise: price * 100 + 20000,
        potIncluded: true,
      },
    ],
  }),
);
export const categories = [...new Set(seedProducts.map((p) => p.category))].map(
  (slug) => ({ slug, name: slug.replaceAll("-", " "), demo: true }),
);
export const guides = [
  {
